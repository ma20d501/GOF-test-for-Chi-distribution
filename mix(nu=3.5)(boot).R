...
# ======================================================================================================
###############################################################
#                 Test for Chi Distribution (nu = 3.5)(Warp-speed bootstrap)
...
# Unified CDF
pchi_family <- function(x, parm, nu = 3.5) {
...
Wn <- function(X, parm) watson_U2_stat(pchi_family(X, parm))
...
for (nu in nu_values) {
  for (n in n_values) {
    ...
    colnames(mac) <- c("Tn(0.5)", "Tn(1)","Tn(1.5)",  
    ...
    for (alt in names(r_alternatives)) {
      ...
      for (i in 1:MC) {
        ...
        s.mat[i,4]<- KSn(x,mu)
        s.boot[i,4]<- KSn(x.boot,mu.boot)
        
        s.mat[i,5]<- CVn(x,mu)
        s.boot[i,5]<-CVn(x.boot,mu.boot)
        
        s.mat[i,6]<- ADn(x,mu)
        s.boot[i,6]<-ADn(x.boot,mu.boot)
        
        s.mat[i,7]<- MAn(x,mu)
        s.boot[i,7]<- MAn(x.boot,mu.boot)
        
        s.mat[i,8]<- Wn(x,mu)
        s.boot[i,8]<- Wn(x.boot,mu.boot)   
        
        s.mat[i,9]<- ZAn(x,mu)
        s.boot[i,9]<- ZAn(x.boot,mu.boot)    
        
        s.mat[i,10]<- ZBn(x,mu)
        s.boot[i,10]<-  ZBn(x.boot,mu.boot)
        
        s.mat[i,11]<- ZCn(x,mu)
        s.boot[i,11]<- ZCn(x.boot,mu.boot)  
