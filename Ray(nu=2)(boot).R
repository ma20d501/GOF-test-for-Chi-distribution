...
# ======================================================================================================
###############################################################
#                 Test for Rayleigh Distribution (nu = 2)(Warp-speed bootstrap)
...
for (nu in nu_values) {
  for (n in n_values) {
    ...
    mac <- matrix(0, nrow = length(r_alternatives), ncol = 17)
    rownames(mac) <- names(r_alternatives)
    colnames(mac) <- c("Tn(0.5)", "Tn(1)","Tn(1.5)","AHn", "MIn", "BKn", "JHn", "PHn", "LAn", 
                       "KSn", "CVn", "ADn", "MAn", "Wn", "ZAn","ZBn","ZCn" )
    
    for (alt in names(r_alternatives)) {
      s.mat <- matrix(0, nrow = MC, ncol = 17)
      s.boot <- matrix(0, nrow = MC, ncol = 17)
      ...
      for (i in 1:MC) {
        ...
        s.mat[i,4] <- AHn(x, mu)
        s.boot[i,4] <- AHn(x.boot, mu.boot)
        
        s.mat[i,5]<- MIn(x,mu)
        s.boot[i,5]<- MIn(x.boot,mu.boot)
        
        s.mat[i,6]<- BKn(x,mu)
        s.boot[i,6]<- BKn(x.boot,mu.boot)
        
        s.mat[i,7] <- JHn(x, mu)
        s.boot[i,7] <- JHn(x.boot, mu.boot)
        
        s.mat[i,8] <- PHn(x, mu)
        s.boot[i,8] <- PHn(x.boot, mu.boot)
        
        s.mat[i,9] <- LAn(x, mu)
        s.boot[i,9] <- LAn(x.boot, mu.boot)
        
        s.mat[i,10]<- KSn(x, mu)
        s.boot[i,10]<- KSn(x.boot,mu.boot)
        
        s.mat[i,11]<- CVn(x, mu)
        s.boot[i,11]<-CVn(x.boot,mu.boot)
        
        s.mat[i,12]<- ADn(x, mu)
        s.boot[i,12]<-ADn(x.boot,mu.boot)
        
        s.mat[i,13]<- MAn(x, mu)
        s.boot[i,13]<- MAn(x.boot,mu.boot)
        
        s.mat[i,14]<- Wn(x, mu)
        s.boot[i,14]<- Wn(x.boot,mu.boot)   
        
        s.mat[i,15]<- ZAn(x,mu)
        s.boot[i,15]<- ZAn(x.boot,mu.boot)    
        
        s.mat[i,16]<- ZBn(x,mu)
        s.boot[i,16]<-  ZBn(x.boot,mu.boot)
        
        s.mat[i,17]<- ZCn(x,mu)
        s.boot[i,17]<- ZCn(x.boot,mu.boot) 
